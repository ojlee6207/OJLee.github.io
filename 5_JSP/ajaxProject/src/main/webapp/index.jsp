...
<bo>
	<h1>Ajax 통신</h1>
	<p>
		<i>"Asynchronous JavaScript And XML"</i>의 약자로<br>
		서버로부터 데이터를 가져와 전체 페이지를 바꾸지 않고 일부만 로드하여 변경하도록 함
		<hr>
		기존에는 a태그를 사용하거나, form태그의 submit 요청 방식은 "동기식" 요청 => 응답페이지가 전달되어야 확인 가능
		<br><br>
		* 동기식 vs 비동기식
		* 동기식
			<ul>
				<li>
					요청 처리 후 해당 응답페이지가 전달된 후에 그 다음 작업 가능
				</li>
				<li>
					서버에 요청한 결과에 따라 응답이 지연되면 무작정 기다린다.(하얀 화면 표시될 것)
				</li>
				<li>
					전체 페이지를 응답하기 때문에 기본적으로 깜박거림이 있음
				</li>
			</ul>
		* 비동기식 (ajax)
			<ul>
				<li>
					현재페이지를 그대로 유지하면서 추가적으로 필요한 요청이 있는 경우 사용
				</li>
				<li>
					요청시 다른 페이지로 연결되지 않음 (현재 페이지 유지)
				</li>
				<li>요청을 하고 해당응답(데이터)이 돌아올떄까지 기다리지 않고 다른 작업 가능</li>
				<li>페이지 깜빡거림이 없음</li>
			</ul>
			ex) 검색어 자동완성, 아이디 중복체크, 찜하기/ 해제하기, 추천, 댓글 등 <br><br>
		* 비동기식 단점 <br>
		<ol>
			<li>현재 페이지에 지속적으로 리소스가 쌓임 => 페이지가 느려짐</li>
			<li>오류 발생시 디버깅 어려움</li>
			<li>요청 처리후 응답된 데이터를 가지고 새로운 요소(태그)를 만들어서 화면에 표시하게 함<br>
			==> DOM 요소들을 새로 만드는 구문을 잘 익혀둬야 함 </li>
		</ol>
		<br><br>
		* AJAX 구현 방식 => 순수 자바스크립트 방식(브라우저마다 방식을 다르게 해야할 수도 있음)
		* jQuery방식(코드가 간결하고 사용하기 쉬움)
	</p>
	...
	<pre>
		*jQuery방식으로 AJAX 통신
		...
		- 부수적인 속성
			...
			+ dataType : 서버에서 응답 시 전달되는 데이터 형 설정, 값이 없다면 자동으로 판단됨
							* xml : 트리 형태의 데이터 구조
	...
	<br>
	응답 : <span id="output1"></span>
 
	<script>
		$(function(){
			$("#btn1").click(function(){
				// 기존 동기식 통신
				// location.href = 'jqAjax1.do?input1=' + $("#input1").val() + '&test=1000';
				...
				$.ajax({
					...
					type: 'get',
					success: function(result){	// 통신 성공 시 실행
						console.log("통신 성공");
					...
					error: function(){		// 통신 실패 시 실행
						console.log("통신 실패")
					},
					complete: function(){	// 통신 성공/실패에 관계없이 무조건 실행
	...
	<br>
	<h3> 2. 버튼 클릭 시 post 방식으로 서버에 여러 개 데이터 전송 및 응답</h3>
		...
		const sendPostData = () => {
			$.ajax({
				...
				success: function(result) {
					...
					*/
					// JSONObject 타입으로 응답시 처리 => { name:"홍길동", age:20}
