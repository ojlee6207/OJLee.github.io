...
<body>
	...
	* 모든 scope 에서 제거<br>
	...
	<table border="1">
		<c:choose>
			...
			<c:otherwise>
				...
				<tbody>
					<c:forEach var="pSon" items="${pList}" varStatus="status">
					<%-- varStatus: 현재 접근된 항목의 상태 정보를 확인할 수 있음 --%>	
						<tr>
							<%-- <td>${ status.index }</td> 	 index 속성 : 0부터 시작 --%>
							<td>${status.count}</td>	<%-- count : 1부터 시작 --%>
							<td>${pSon.name}</td>
							<td>${pSon.age}</td>
							<td>${pSon.gender}</td>
						</tr>
	...
	<pre>
		* (c:forTokens var="변수명" items="분리하고자 하는 문자열" delims="구분자")
		
		- 구분자를 통해서 분리된 각각의 문자열에 순차적으로 접근하여 반복문을 수행
		- 콤마(,)가 아닌 다른 구분자로 문자열을 분리하고자 할 때 사용
		- Java에서 문자열.split("구분자") 또는 StringTokenizer와 비슷한 기능
	</pre>
	
	<%-- movie 변수에 5개 작성(구분자:, . / --%>
	
	<c:set var="movie" value="파묘,듄2.킹덤 오브 헤븐/8월의 크리스마스,웡카" />
		 
	<%-- 순서있는 목록 태그로 영화 목록 출력 --%>
	<h4>영화목록</h4>
	<ol>
	...
	<pre>
		...
		* query String : ex) localhost:8003/ea/list.bo?**cpage=3**
		c:url var="변수명" value="요청할 url주소"
			c:param name="키값" value="데이터값" /
		/c:url
	</pre>
	...
	<a href="${ listUrl }">jstl(c:url)을 사용한 방식</a><br>
